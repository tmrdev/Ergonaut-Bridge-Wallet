...
var ErgoBox = class _ErgoBox {
  ...
  get boxId() {
    if (!this.#boxId) {
      this.#boxId = crypto.hex.encode(crypto.blake2b256(serializer.serializeBox(this).toBytes()));
    }
    return this.#boxId;
  }
  get transactionId() {
  ...
  constructor(box, transactionId, index) {
    if (box instanceof _ErgoBox) {
      ...
      if (!transactionId || common.isUndefined(index)) {
        throw new common.FleetError("TransactionId and Index must be provided for Box generation.");
      }
...
function isBox(box) {
  const castedBox = box;
  return !!castedBox.boxId && !!castedBox.transactionId && common.isDefined(castedBox.index);
}
...
var ErgoUnsignedInput = class extends ErgoBox {
  ...
  setContextExtension(extension) {
    for (const key in extension) {
      ...
      if (!c) continue;
      this.#extension[key] = typeof c === "string" ? c : c.toHex();
    }
    return this;
  }
  /**
  ...
  toDataInputPlainObject(type) {
    return super.toPlainObject(type);
  }
};

// src/models/ergoUnsignedTransactionChain.ts
var ErgoUnsignedTransactionChain = class {
  #entryPoint;
...
// src/models/ergoUnsignedTransaction.ts
var ErgoUnsignedTransaction = class {
  ...
  #id;
  #builder;
  constructor(inputs, dataInputs, outputs, builder) {
    this.#inputs = inputs;
  ...
  get id() {
    if (!this.#id) {
      this.#id = crypto.hex.encode(crypto.blake2b256(this.toBytes()));
    }
    return this.#id;
  }
  get inputs() {
  ...
  get burning() {
    const diff = common.utxoDiff(common.utxoSum(this.inputs), common.utxoSum(this.#outputCandidates));
  ...
  chain(callback) {
    if (!this.#builder) {
      throw new common.FleetError("Cannot chain transactions without a parent TransactionBuilder");
    }
...
var ErgoTree = class {
  ...
  constructor(input, network) {
    this.#byteReader = new serializer.SigmaByteReader(input);
    this.#header = this.#byteReader.readByte();
    this.#network = network ?? common.Network.Mainnet;
  }
  get bytes() {
  ...
  replaceConstant(index, constant) {
  ...
  serialize() {
    ...
    let constantsSize = 0;
    if (segregatedFlag) {
      constantsSize = serializer.estimateVLQSize(this.#constants.length) + this.#constants.reduce((acc, constant) => acc + constant.bytes.length, 0);
    }
    const treeSize = constantsSize + this.#root.length;
    let totalSize = HEADER_SIZE + treeSize;
    if (sizeFlag) totalSize += serializer.estimateVLQSize(totalSize);
    const writer = new serializer.SigmaByteWriter(totalSize).write(this.#header);
  ...
  #parse() {
    ...
    if (this.hasSize) this.#byteReader.readUInt();
    if (!this.hasSegregatedConstants) {
      this.#root = this.#byteReader.readRemainingBytes();
    } else {
      this.#constants = this.#byteReader.readArray((r) => serializer.SConstant.from(r));
      this.#root = this.#byteReader.readRemainingBytes();
    }
    return this;
  }
...
var OutputBuilder = class _OutputBuilder {
  #address;
  #tokens;
  #value;
  #valueEstimator;
  ...
  constructor(value, recipient, creationHeight) {
    ...
    if (typeof recipient === "string") {
      this.#address = common.isHex(recipient) ? ErgoAddress.fromErgoTree(recipient) : ErgoAddress.fromBase58(recipient);
  ...
  get value() {
    return common.isDefined(this.#valueEstimator) ? this.#valueEstimator(this) : this.#value;
  }
  get address() {
    return this.#address;
  }
  ...
  static from(box) {
    return new _OutputBuilder(box.value, box.ergoTree).addTokens(box.assets).setAdditionalRegisters(box.additionalRegisters);
  }
  setValue(value) {
    if (typeof value === "function") {
      this.#valueEstimator = value;
    } else {
      this.#value = common.ensureBigInt(value);
      this.#valueEstimator = void 0;
      if (this.#value <= common._0n) {
  ...
  setCreationHeight(height, options) {
    if (common.isUndefined(options) || options.replace === true || options.replace === false && common.isUndefined(this.#creationHeight)) {
  ...
  setAdditionalRegisters(registers) {
    for (const key in registers) {
      ...
      if (!r) continue;
      this.#registers[key] = typeof r === "string" ? r : r.toHex();
    }
    return this;
  }
  eject(ejector) {
  ...
  build(transactionInputs) {
    if (!common.areRegistersDenselyPacked(this.#registers)) throw new InvalidRegistersPacking();
    let tokens;
    if (this.minting) {
      ...
      tokens = this.assets.toArray().filter((x) => x.amount > common._0n);
    }
...
var TransactionBuilder = class {
  ...
  from(inputs, options = { ensureInclusion: false }) {
    ...
    if (!this.#ensureInclusion) this.#ensureInclusion = /* @__PURE__ */ new Set();
  ...
  sendChangeTo(address) {
    if (typeof address === "string") {
      this.#changeAddress = common.isHex(address) ? ErgoAddress.fromErgoTree(address, common.Network.Mainnet) : ErgoAddress.fromBase58(address);
    } else {
  ...
  payFee(amount) {
    this.#feeAmount = common.ensureBigInt(amount);
    return this;
  }
  payMinFee() {
    this.payFee(common.RECOMMENDED_MIN_FEE_VALUE);
    return this;
  }
  burnTokens(tokens) {
  ...
  configureSelector(selectorCallback) {
    if (common.isUndefined(this.#selectorCallbacks)) this.#selectorCallbacks = [];
  ...
  build() {
    ...
    this.outputs.toArray().map((output) => output.setCreationHeight(this.#creationHeight, { replace: false }));
    const outputs = this.outputs.clone();
    if (common.isDefined(this.#feeAmount)) {
      outputs.add(new OutputBuilder(this.#feeAmount, common.FEE_CONTRACT));
    }
    ...
    if (common.some(this.#selectorCallbacks)) {
      for (const selectorCallBack of this.#selectorCallbacks) {
        selectorCallBack(selector);
      }
    }
    const target = common.some(this.#burning) ? outputs.sum({ tokens: this.#burning.toArray() }) : outputs.sum();
    let inputs = selector.select(target);
    if (common.isDefined(this.#changeAddress)) {
      const changeBoxes = [];
      ...
      if (common.some(change.tokens)) {
        ...
        const chunkedTokens = common.chunk(change.tokens, this.#settings.maxTokensPerChangeBox);
        for (const tokens of chunkedTokens) {
          const output = new OutputBuilder(estimateMinBoxValue(), this.#changeAddress).setCreationHeight(this.#creationHeight).addTokens(tokens).setFlags({ change: true });
      ...
      if (change.nanoErgs > common._0n) {
        if (!changeBoxes.length || this.settings.shouldIsolateErgOnChange) {
          changeBoxes.unshift(
            new OutputBuilder(change.nanoErgs, this.#changeAddress).setCreationHeight(this.#creationHeight).setFlags({ change: true })
          );
        } else {
    ...
    const buildedOutputs = outputs.toArray().map(
      (output) => output.setCreationHeight(this.#creationHeight, { replace: false }).build(inputs)
    );
    ...
    if (common.some(burning.tokens) && common.some(this.#burning)) {
      burning = common.utxoDiff(burning, {
        nanoErgs: common._0n,
        tokens: this.#burning.toArray()
      });
    }
    if (!this.#settings.canBurnTokens && common.some(burning.tokens)) {
  ...
  #isMinting() {
    return this.#getMintingOutput() !== void 0;
  }
  #getMintingTokenId() {
    return this.#getMintingOutput()?.minting?.tokenId;
  }
  #isMoreThanOneTokenBeingMinted() {
    let mintingCount = 0;
    for (const output of this.#outputs) {
      if (output.minting) {
        mintingCount++;
  ...
  #isTheSameTokenBeingMintedFromOutsideTheMintingBox() {
...
Object.defineProperty(exports, "FEE_CONTRACT", {
  enumerable: true,
  get: function () { return common.FEE_CONTRACT; }
});
Object.defineProperty(exports, "Network", {
  enumerable: true,
  get: function () { return common.Network; }
});
Object.defineProperty(exports, "RECOMMENDED_MIN_FEE_VALUE", {
  enumerable: true,
  get: function () { return common.RECOMMENDED_MIN_FEE_VALUE; }
});
